...
Author: Guohua Zhang
Date: 2025.2.10
Description: This script is used to deploy the RL policy in mujoco simulator for MI4.
Version: 1.0
'''
...
_gait_command[1] = 0.3 # Contact duration range [0-1]
# _gait_command[1] = 0.4
...
def run_mujoco(policy, cfg):
    ...
    for i in range(int(cfg.sim_config.sim_duration / cfg.sim_config.dt)):
        ...
        if count_lowlevel % cfg.sim_config.decimation == 0:
            ...
            # target_q = cfg.robot_config.default_dof_pos
...
if __name__ == '__main__':
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Deployment script of MI4 in mujoco simulator.')
    parser.add_argument('--load_model', type=str, default='./policy/mi4/policy.pt', help='Run to load from.')
    parser.add_argument('--robot_load_path', type=str, default="./robot-descriptions/mi4/xml/", help='Robot load path.')
    ...
    args.load_model = './policy/mi4/policy.pt'
    args.terrain = True
    args.control_mode = 'keyboard'
    args.terrain_path = 'world_heightfield.xml'
    args.robot_load_path = './robot-descriptions/mi4/xml/'
    ...
    class Sim2simCfg:
        ...
        class robot_config:
            ...
            tau = 360.0
            tau_calf = 360.0
            kps = np.array([hip_kp, thigh_kp, calf_kp, hip_kp, thigh_kp, calf_kp, hip_kp, thigh_kp, calf_kp, hip_kp, thigh_kp, calf_kp], dtype=np.double)
            kds = np.array([hip_kd, thigh_kd, calf_kd, hip_kd, thigh_kd, calf_kd, hip_kd, thigh_kd, calf_kd, hip_kd, thigh_kd, calf_kd], dtype=np.double)
            tau_limit = np.array([tau, tau, tau_calf, tau, tau, tau_calf, tau, tau, tau_calf, tau, tau, tau_calf], dtype=np.double)
            default_dof_pos = np.array([
                0.0, 0.71, -1.51, 0.0, 0.71, -1.51, 0.0, 0.71, -1.51, 0.0, 0.71, -1.51
            ])
            default_dof_pos = default_dof_pos[dofmapping_mujoco2lab]
            tau_limit = tau_limit[dofmapping_mujoco2lab]
            kps = kps[dofmapping_mujoco2lab]
            kds = kds[dofmapping_mujoco2lab]
