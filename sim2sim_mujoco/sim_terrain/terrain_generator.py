...
ROBOT = "go1"
INPUT_SCENE_PATH = "./input/" + ROBOT + "/scene.xml"
OUTPUT_SCENE_PATH = "./output/" + ROBOT + "/scene_terrain.xml"
TERRAIN_IMAGE_PATH = "./output/" + ROBOT
XML_USAGE_PATH = f"{LEGGED_GYM_ROOT_DIR}/resources/robots/" + ROBOT + "/xml/scene_terrain.xml"
IMAGE_USAGE_PATH = f"{LEGGED_GYM_ROOT_DIR}/resources/robots/" + ROBOT + "/height_field.png"
...
class TerrainGenerator:
    ...
    def AddPerlinHeighField(
        ...
        cv2.imwrite(IMAGE_USAGE_PATH, terrain_image)
        cv2.imwrite(TERRAIN_IMAGE_PATH + "/" + output_hfield_image,
                    terrain_image)
    ...
    def Save(self):
        ...
        self.scene.write(XML_USAGE_PATH)
...
if __name__ == "__main__":
    ...
    # Stairs
    tg.AddStairs(init_pos=[4.5, 0.0, 0.0], length=3.0, yaw=0.0, stair_nums=30)
    tg.AddStairs(init_pos=[4.5, -3.0, 0.0], length=3.0, yaw=0.0, stair_nums=30)
    tg.AddStairs(init_pos=[4.5, 3.0, 0.0], length=3.0, yaw=0.0, stair_nums=30)
    tg.AddStairs(init_pos=[4.5, 6.0, 0.0], length=3.0, yaw=0.0, stair_nums=30)

    # # Suspend stairs
    # tg.AddSuspendStairs(init_pos=[1.0, 6.0, 0.0], yaw=0.0)

    # # Rough ground
    tg.AddRoughGround(init_pos=[-1.0, -3.0, 0.0],
                      box_size=[0.8, 0.8, 0.8],
                      euler=[0, 0, 0.0],
                      nums=[50, 50])

    # Perlin heigh field
    # tg.AddPerlinHeighField(position=[-1.5, 4.0, 0.0], size=[2.0, 1.5])
    # tg.AddPerlinHeighField(position=[0.0, 0.0, 0.0], height_scale=0.5, size=[20.0, 20])
